...
products.forEach(function (product) {
  ...
  if (addButton) {
    addButton.addEventListener("click", function () {
      ...
      price = Number(price.replace("₹", "").replace(",", "").trim());
...
if (detailButton) {
  detailButton.addEventListener("click", function () {
    ...
    addProductToCart(image, name, price, quantity);
  });
}
...
function updateCartPopup() {
  ...
  let popups = document.querySelectorAll(".cart-popup");

  for (let p = 0; p < popups.length; p++) {
    let popupProducts = popups[p].querySelector("#popupProducts");

    let popupSubtotal = popups[p].querySelector("#popupSubtotal");
    ...
    if (cart.length === 0) {
      popupProducts.innerHTML = "<p>No products in the cart.</p>";
    }

    popupSubtotal.textContent = "₹" + total.toFixed(2);
  }
}
...
for (let i = 0; i < cartBoxes.length; i++) {
  ...
  let popup = cartBox.querySelector(".cart-popup");
  ...
  let cartLink = cartBox.querySelector("a[href='cart.html']");
  ...
  popup.addEventListener("click", function (event) {
    if (!event.target.classList.contains("remove-popup-item")) {
      return;
    }
